  ...
  for (var i = 0; i < dataInbox.length; i++) {
    ...
    if (estadoInboxActual === "Pendiente") {
      ...
      if (row[19] instanceof Date) {
        periodoInforme = Utilities.formatDate(row[19], Session.getScriptTimeZone(), "yyyy-MM");
      } else if (typeof row[19] === "number") {
        var fechaDesdeSerial = new Date(Math.round((row[19] - 25569) * 86400 * 1000));
        periodoInforme = Utilities.formatDate(fechaDesdeSerial, Session.getScriptTimeZone(), "yyyy-MM");
      } else {
        var rawPeriodo = String(row[19] || "").trim();

        if (/^\d+$/.test(rawPeriodo)) {
          var serial = Number(rawPeriodo);
          var fechaDesdeSerialStr = new Date(Math.round((serial - 25569) * 86400 * 1000));
          periodoInforme = Utilities.formatDate(fechaDesdeSerialStr, Session.getScriptTimeZone(), "yyyy-MM");
        } else {
          periodoInforme = rawPeriodo;
        }
      }
      var equiposTexto       = String(row[20] || "").trim();
      
      // Limpiar posibles contaminaciones tipo "46113|UI-1.1"
      if (equiposTexto.includes("|")) {
        var parts = equiposTexto.split("|");
        equiposTexto = parts[parts.length - 1].trim();
      }
      ...
      // El estado SIEMPRE es "Pendiente" por defecto al entrar a validación.
      // Nunca asume "Validado" automáticamente.
      var nuevoEstadoAprobacion = "Pendiente";
      var comentarioValidacion = "";
      
      if (faltantes.length > 0) {
        // Solo agrega comentario si hay información faltante
